...
netPresentValue rate cashFlows = foldl' (go rate)
                                        0
...
presentValue rate nper pmt fv payAtBeginning | rate /= 0 = pv
                                             | otherwise = pv'
  where
    a       = (1 + rate) ^ nper
...
futureValue rate nper pmt pv payAtBeginning | rate /= 0 = fv
                                            | otherwise = fv'
  where
    a       = (1 + rate) ^ nper
...
payment rate nper pv fv payAtBeginning | rate /= 0 = pmt
                                       | otherwise = pmt'
  where
    a       = (1 + rate) ^ nper
...
amortizedPayments rate nper pv fv payAtBeginning p
    ...
    where pmts = fmap (paymentParts rate nper pv fv payAtBeginning) [p .. nper]
